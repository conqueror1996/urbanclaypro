...
export default function Projects({ projects = [], showLink = true }: ProjectsProps) {
    return (
        <section id="projects" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 md:py-48">
            ...
            <div className="mt-12 flex overflow-x-auto snap-x snap-mandatory gap-6 pb-8 -mx-4 px-4 sm:grid sm:grid-cols-2 lg:grid-cols-3 sm:overflow-visible sm:pb-0 sm:mx-0 sm:px-0 scrollbar-hide">
                {projects.map((p, index) => (
                    <Link href={`/projects/${p.slug || '#'}`} key={index} className="block min-w-[85vw] sm:min-w-0 snap-center h-full">
                        <motion.article
