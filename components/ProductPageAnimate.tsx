    ...
    return (
        <main className="bg-[var(--background)] text-[var(--foreground)] min-h-screen selection:bg-[var(--terracotta)] selection:text-white pb-0 lg:pb-0 overflow-x-hidden w-full relative">
            ...
            <div className="fixed bottom-0 left-0 right-0 z-50 bg-[var(--background)]/90 backdrop-blur-xl border-t border-[var(--line)] shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
                <div className="max-w-[1800px] mx-auto w-full px-4 md:px-8 py-2 md:py-4 flex flex-col md:flex-row items-center justify-between gap-2 md:gap-4">

                    {/* Price Display - Responsive */}
                    <div className="flex md:flex-col items-center md:items-start justify-between md:justify-center w-full md:w-auto px-1 md:px-0">
                        <div className="flex items-baseline gap-1.5">
                            <span className="text-lg md:text-2xl font-serif text-[var(--foreground)]">
                                {(() => {
                            ...
                            {product.priceRange && <span className="text-[9px] md:text-xs text-[var(--foreground)]/70 font-light lowercase">/ sq.ft</span>}
                        </div>
                        <span className="text-[7px] md:text-[9px] text-[var(--foreground)]/70 font-bold tracking-widest uppercase md:mt-0.5">*Quantity based pricing</span>
                    </div>

                    {/* Buttons Container */}
                    <div className="flex w-full md:w-auto gap-2 items-center h-11 md:h-14">
                        {/* Quick WhatsApp Support / Brand Icon */}
                        <a
                            href={`https://wa.me/919930495393?text=Hi, I am looking at ${product.title}. Can you help me with a technical query?`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="w-11 md:w-14 h-full flex items-center justify-center bg-[var(--line)]/50 hover:bg-[var(--line)] border border-[var(--line)] rounded-xl text-[#25D366] active:scale-95 transition-all shadow-sm shrink-0"
