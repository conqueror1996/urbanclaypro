...
interface ProjectAtlasMapProps {
    projects: any[];
}

export default function ProjectAtlasMap({ projects = [] }: ProjectAtlasMapProps) {
    ...
    return (
        ...
        >
            <svg viewBox="0 0 600 700" className="w-full h-full drop-shadow-2xl filter">
                ...
                />
                <path d="M225.5,38.5 L210,20 L230,10 L250,15 L265.5,35.5 Z" fill="url(#mapGradient)" stroke="var(--terracotta)" strokeWidth="0.5" />
                    ...
                    <AnimatePresence>
                        {activeLocation === loc.id && (
                            <motion.div
                                ...
                                className="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 w-[240px] md:w-72 bg-[var(--ink)]/95 backdrop-blur-xl rounded-xl border border-white/10 shadow-2xl p-3 md:p-4 z-50 pointer-events-none origin-bottom"
                            >
                                <div className="flex gap-3 md:gap-4">
                                    ...
                                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                                        <div className="flex items-center gap-2 mb-0.5 md:mb-1">
                                            ...
                                            <span className="text-[9px] md:text-[10px] font-bold text-white/80 uppercase tracking-wider">{loc.city}</span>
                                        </div>
                                        <h4 className="text-sm md:text-base font-serif text-white leading-tight mb-0.5 md:mb-1 truncate">{loc.project.name}</h4>
                                        <p className="text-[10px] md:text-xs text-white/70 leading-snug line-clamp-2">{loc.project.details}</p>
                                    </div>
                                </div>
                                <div className="absolute top-full left-1/2 -translate-x-1/2 -mt-px border-8 border-transparent border-t-[var(--ink)]/95"></div>
                            </motion.div>
                        )}
                    </AnimatePresence>
                </Link>
